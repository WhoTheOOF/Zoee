...
class Welcome(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        if member.guild.id == 1192455862464299058:
            ...
            embed_w.set_author(name=f"⇾ Ciao {member.name} ⇽", icon_url=member.avatar.url if member.avatar != None else member.guild.icon.url)
            embed_w.set_image(url= member.guild.banner.url if member.guild.banner != None else "https://cdn.discordapp.com/attachments/1192457322337947748/1192488696398749746/server_banner.gif?ex=65a942a4&is=6596cda4&hm=2a01caf8596219071e01cbd25356494f93e11958594c58b82644d471ef3be6c3&")
            ...
            embed_g.set_thumbnail(url=member.avatar.url if member.avatar != None else member.guild.icon.url)
